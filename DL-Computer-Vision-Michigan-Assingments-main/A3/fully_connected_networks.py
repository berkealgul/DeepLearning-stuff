...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['W1'] = torch.normal(mean=0.0, std=weight_scale, size=(input_dim, hidden_dim))
        self.params['b1'] = torch.zeros(size=(hidden_dim, ))
        self.params['W2'] = torch.normal(mean=0.0, std=weight_scale, size=(hidden_dim, num_classes))
        self.params['b2'] = torch.zeros(size=(num_classes, ))
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_2 = Linear_ReLU.forward(scores, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dx, grads['W2'], grads['b2'] = Linear_ReLU.backward(dx, cache_2)
        dx, grads['W2'], grads['b2'] = Linear_ReLU.backward(dx, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        - dropout: Scalar between 0 and 1 giving the drop probability
          for netw          scores, cache_2 = Linear_ReLU.forward(scores, self.params['W2'], self.params['b2']) you should use double for numeric gradient checking.
        - device: device to use for computation. 'cpu' or 'cuda'
        """
        ...
        for hidden_dim in hidden_dims:
          i += 1
          self.params['W{}'.format(i)] = torch.normal(mean=0.0, std=weight_scale, size=(previous_dim, hidden_dim), device=device, dtype=dtype)
          self.params['b{}'.format(i)] = torch.zeros(size=(hidden_dim, ), dtype=dtype)
          previous_dim = hidden_dim
        i += 1
        self.params['W{}'.format(i)] = torch.normal(mean=0.0, std=weight_scale, size=(previous_dim, num_classes))
        self.params['b{}'.format(i)] = torch.zeros(size=(num_classes, ))
    ...
    def loss(self, X, y=None):
        ...
        dropout_caches = []
        scores = X
        for i in range(i, self.num_layers+1):
          scores, cache = Linear_ReLU.forward(scores, self.params['W{}'.format(i)], self.params['b{}'.format(i)])
          caches.append(cache)
          if self.use_dropout:
            scores, d_cache =  Dropout.forward(scores, self.dropout_param)
        ...
        for i in range(self.num_layers-1, 0, -1):
          ...
          b_key = 'b{}'.format(i)
          if self.use_dropout:
            dx = Dropout.backward(dx, dropout_caches[i])
          dx, grads[w_key], grads[b_key] = Linear_ReLU.backward(dx, caches[i]) 
          loss += self.reg * torch.sum(self.params[w_key]*self.params[w_key])

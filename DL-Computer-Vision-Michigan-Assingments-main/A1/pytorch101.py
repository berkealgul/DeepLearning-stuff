...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for v, (i, j) in enumerate(indices): x[i, j] = values[v]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    for s in x.size(): num_elements *= s
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if start >= stop:
        x = torch.empty(size=(0,), dtype=torch.float64)
    else:
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    # Replace "pass" statement with your code
    last_row = x[-1,:]
    third_col = x[:, 2].reshape(x.shape[0], 1)
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x[:2,:3] = torch.arange(0, 3, 1).reshape(1,3).expand(2,3)
    x[:2,3:6] = 2
    x[2:4,:3:2] = 3
    x[2:4,1:4:2] = 4
    x[2:4,4:6] = 5

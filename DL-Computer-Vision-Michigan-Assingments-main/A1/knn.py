...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        x_train_f = torch.flatten(x_train[i])
        for j in range(num_test):
            x_test_f = torch.flatten(x_test[j])
            dists[i, j] = torch.sum((x_train_f - x_test_f)**2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_test_f  = torch.flatten(x_test,  start_dim=1)
    x_train_f = torch.flatten(x_train, start_dim=1)
    for i in range(num_train):
        dists[i] = torch.sum((x_train_f[i] - x_test_f)**2, axis=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dists -= 2 * (x_train @ x_test.T)
    dists += torch.sum(x_train**2, axis=1, keepdims=True)
    dists += torch.sum(x_test**2, axis=1, keepdims=True).T
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        _, indices = torch.topk(input=dists[:,i], k=k, largest=False)
        y_pred[i] = torch.mode(y_train[indices])[0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_fold = x_train_folds[i]
            y_fold = y_train_folds[i]
            x_train = torch.cat([ x_train_folds[j] for j in range(num_folds) if j != i])
            y_train = torch.cat([ y_train_folds[j] for j in range(num_folds) if j != i])
            knn = KnnClassifier(x_train, y_train)
            acc = knn.check_accuracy(x_fold, y_fold, k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    highest_avg = 0
    for k, scores in k_to_accuracies.items():
        avg_acc = sum(scores)/len(scores)
        if avg_acc > highest_avg:
            best_k = k
            highest_avg = avg_acc
        elif avg_acc == highest_avg and k < best_k:
            best_k = k
            highest_avg = avg_acc
    ##########################################################################
    #                           END OF YOUR CODE                             #

...
def svm_get_search_params():
    ...
    learning_rates = torch.range(start=1e-3, end=1e-1, step=(1e-2-1e-5)/4)
    regularization_strengths = torch.range(start=1e-1, end=1e1, step=(1e2-1e-1)/4)
...
def test_one_param_set(
    ...
    batch_size = 200
    cls.train(data_dict['X_train'], data_dict['y_train'], lr, reg, num_iters, batch_size)
    
    y_train_pred = cls.predict(data_dict['X_train'])
    train_acc = y_train_pred[y_train_pred == data_dict['y_train']].numel() / y_train_pred.shape[0]

    y_val_pred = cls.predict(data_dict['X_val'])
    val_acc = y_val_pred[y_val_pred == data_dict['y_val']].numel() / y_val_pred.shape[0]
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = np.linspace(1e-5, 2e-1, 10) # [1e-2, 5e-1] 
    regularization_strengths = np.linspace(1e-6, 1e-1, 10)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################

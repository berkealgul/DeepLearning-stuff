...
def nn_get_search_params():
    ...
    learning_rate_decays = [0.99, 0.98, 0.97, 0.96]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lr_dec in learning_rate_decays:
                    print("Training with lr: %.2f, hs: %.2f, reg: %.2f, lr_dec: %.2f | best_val_acc: %.2f" % (lr, hs, reg, lr_dec, best_val_acc))
                    net = TwoLayerNet(3 * 32 * 32, hs, 10, device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)
                    stat = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'],
                                num_iters=3000, batch_size=1000,
                                learning_rate=lr, learning_rate_decay=lr_dec,
                                reg=reg, verbose=False)
                    val_acc = max(stat["val_acc_history"]) 
